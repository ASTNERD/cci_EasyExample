...
//Das ist meine Favorisierte Schreibweise. man muss sich aber das ? und den : f�r WENN und SONST einpr�gen.
...
void VTC_handleSoftkeysAndButtons_RELEASED(const struct ButtonActivation_S *pButtonData) {
	...
	case Button_MinusMinus:
		//bei Z�hlerstand 0 soll er nicht mehr -- machen.
		if (Tageszaehler > 0)
			Tageszaehler--;
		//bei Z�hlerstand 0 soll er nicht mehr -- machen.
		if (Gesamtzaehler > 0)
	...
	// Speichern des Tageszaehler; non-volatile; nicht fl�chtig; spannungsausfallsicher gespeichert
	setU32("CF-A", "Tageszaehler", Tageszaehler);
	// Speichern des Gesamtzaehler; non-volatile; nicht fl�chtig; spannungsausfallsicher gespeichert
...
void VTC_handleNumericValues(const struct InputNumber_S * pInputNumberData) {
	...
	case NumberVariable_Gesamtziel:
		// Variable mit dem Namen NumberVariable_Gesamtziel wurde vom Benutzer am UT Ver�ndert
		...
		// Speichern des Gesamtziel; non-volatile; nicht fl�chtig; spannungsausfallsicher gespeichert
		setU32("CF-A", "Gesamtziel", Gesamtziel);
		break;

	case NumberVariable_Tagesziel:
		// Variable mit dem Namen NumberVariable_Tagesziel wurde vom Benutzer am UT Ver�ndert
		...
		// Speichern des Tagesziel; non-volatile; nicht fl�chtig; spannungsausfallsicher gespeichert
